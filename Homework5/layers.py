...
def affine_forward(x, w, b):
  ...
  #   assignments.
  # ================================================================ #
  D=np.prod(x.shape[1:]); N=x.shape[0]
  X= x.reshape(N, D)
  out= X@w + b[np.newaxis, :]
  # ================================================================ #
  # END YOUR CODE HERE
  # ================================================================ #
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    x_hat = (x-batch_mean)/np.sqrt(batch_var+eps) # normalized x
    out = x_hat*gamma[np.newaxis, :] + beta[np.newaxis, :]
    
    cache={}; cache['mu']= batch_mean; cache['var']= batch_var
    cache['eps']= eps; cache['x_hat']= x_hat; cache['gamma'] = gamma
    cache['x']= x
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    ...
    running_mean = momentum*running_mean + (1-momentum)*batch_mean
    running_var = momentum*running_var + (1-momentum)*batch_var
    x_hat = (x-running_mean)/np.sqrt(running_var+eps) # normalized x
    out=x_hat
    out = x_hat*gamma[np.newaxis, :] + beta[np.newaxis, :]
    # ================================================================ #
    # END YOUR CODE HERE
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
  x_hat= cache['x_hat']; mu= cache['mu']; var= cache['var']; eps= cache['eps']
  gamma= cache['gamma']; x= cache['x']
  N= dout.shape[0]

  dbeta= np.sum(dout, axis=0)
  dgamma= np.sum(dout*x_hat, axis=0) # dot product along the N axis
  dx_hat= dout*gamma
  dmu= -1/np.sqrt(var+eps)*np.sum(dx_hat, axis=0)
  dvar= np.sum( -0.5/(var+eps)**1.5 * (x-mu)*dx_hat , axis=0 )
  dx= 1/np.sqrt(var+eps)*dx_hat + 2/N*(x-mu)*(dvar) + dmu/N
  # ================================================================ #
  # END YOUR CODE HERE
  # ================================================================ #
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    # ================================================================ #
    out= x # if mode is test, return the input
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
