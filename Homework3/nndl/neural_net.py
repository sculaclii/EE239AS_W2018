...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # W1~H,D  X~N,D  b1~H  a1=W1*X+b1  l1out=ReLU(a1)
    a1= W1@X.T + b1[:, np.newaxis] # a1~H,N
    l1out=np.maximum(a1, 0); # ReLU, ~H,N
    # W2~C,H
    a2= W2@l1out + b2[:,np.newaxis] #a2~C,N
    scores=a2.T
    ...
    # l2out~C,N
    mask=np.zeros_like(a2, dtype=bool); mask[y, range(N)]=True
    loss= np.sum( ( np.log(np.sum(np.exp(a2),0)) - a2[mask] ) )/N + \
        + reg*0.5*( np.sum(np.square(W1)) + np.sum(np.square(W2)) )
    ...
    # NN: X->W1,b1 = a1->ReLU = l1out->W2,b2 = a2->softmaxL = Loss
    # W2~C,H  b2~C  fc2~N,C  a2~C,N
    softProb = np.exp(a2)/ np.sum(np.exp(a2),0)[np.newaxis,:] #~C,N
    mask=np.zeros_like(a2, dtype=bool); mask[y, range(N)]=True
    dLda2 = softProb - mask*1.0 #~C,N
    N=X.shape[0]
    grads['b2'] = np.sum(dLda2,1) / N #~C
    grads['W2'] = dLda2@l1out.T /N + reg*W2 #~C,H
    dLdl1out = W2.T@dLda2 #~H,N
    dLda1 = (a1>0)*dLdl1out #~H,N
    grads['b1'] = np.sum(dLda1,1) /N #~H
    grads['W1'] = dLda1@X /N + reg*W1 #~H,D
